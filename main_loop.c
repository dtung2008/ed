...
void set_prompt(const char *s) {
  prompt_on = 1;
  strncpy(prompt_str, s[0] == '='? &s[1]: s, sizeof(prompt_str));
...
static int exec_command(const char isglobal) {
  ...
  ibufp = skip_blanks(ibufp);
  c = *ibufp++;
  printf("%c %d cnt=%d f=%d s=%d g=%d\n", c, c, addr_cnt, first_addr, second_addr, isglobal);
  switch(c) {
...
int main_loop(const char loose) {
  jmp_buf jmp_state;
  int err_status = 0;			/* program exit status */
  int linenum = 0;			/* script line number */
